...
import { validateCalendarDesign } from '@buildcalendar/calendar-core';
...
const TEMPLATES = [
  { name: 'Kayu', slug: 'kayu', presetCode: 'WALL-12', category: 'minimal' },
  { name: 'Pesisir', slug: 'pesisir', presetCode: 'DESK-A5L', category: 'photo' },
  { name: 'Batik Modern', slug: 'batik-modern', presetCode: 'WALL-1', category: 'pattern' },
];
...
async function seedTemplates() {
  ...
  for (const [index, template] of TEMPLATES.entries()) {
    ...
    // The Design JSON is a real file in `packages/db/templates`, generated by
    // `pnpm --filter @buildcalendar/db templates`. Validating it here means a
    // broken launch template fails the seed rather than the first customer.
    const designFile = path.join(TEMPLATE_DIR, `${template.slug}.json`);
    const design: unknown = JSON.parse(readFileSync(designFile, 'utf8'));

    const result = validateCalendarDesign(design, {
      code: preset.code,
    ...
    if (!result.valid || !result.slotSchema) {
      throw new Error(
        `${template.slug} is not a valid design: ` +
          result.issues
    ...
    const data = {
      ...
      // Prisma types Json columns as its own InputJsonValue; the derived schema is
      // plain data and satisfies it structurally.
      slotSchema: result.slotSchema as unknown as object,
      isPremium: false,
      // Inactive until an admin has uploaded the design to R2 through the panel
      // and previewed it (P1-US-702). A row whose design_key points at nothing
      // must never appear in the public gallery.
      isActive: false,
      sortOrder: index + 1,
    };
  ...
  console.log(
    `  templates        ${seeded} (validated, inactive until imported through the admin panel)`,
  );
}
